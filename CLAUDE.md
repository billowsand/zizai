...
Windows 输入法（本项目是从上游青简 fork 出来的 Windows 专版）：Core 平台无关，Windows 壳是唯一的壳。
...
- `crates/qingjian-core`：引擎。`Engine` 是对外唯一门面，`Translator` / `Learner` 等 trait 在 `engine` 模块；拼音解析、纠错、候选、排序、整句、双拼、英文模式都在这里。
...
- `crates/qingjian-render`：自绘渲染器：候选窗与悬浮状态条一帧 + 色系 → 位图，Windows Server 贴图。入口 `Renderer` / `Theme`，见 `docs/design/rendering.md`。
- `crates/qingjian-voice`：Windows 本地语音输入的 SenseVoice、麦克风采集、重采样与 Worker 私有协议；实现源自 auto-voice（MIT）。
- `apps/cli`：Core 的验证工具：查询、逐键计时、输入日志回放、整句评测、常数扫描。排序 / 整句 / 纠错的改动先跑它再合。
- `apps/windows`：`server`（Server 进程：Engine + IPC + 自绘候选窗与状态条）+ `voice-worker`（按需采音与本地识别）+ `tsf`（TSF DLL）+ `settings-egui`（缺省设置程序）+ `settings`（WinUI 3 对比实现）+ `installer`（Inno）。DLL 不能带 Engine、音频或模型依赖树。
...
powershell -File apps/windows/installer/build.ps1   # 打 Windows 安装包（部署与真机测试见 apps/windows/README.md）
```
