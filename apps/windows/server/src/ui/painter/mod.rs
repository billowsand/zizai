//! 字在渲染器在 Windows 壳里的落地：字体库 + 渲染器一份，候选窗口与状态条共用（字形缓存共享）。

use std::cell::RefCell;
use std::rc::Rc;

use qingjian_platform::ColorScheme;
use qingjian_render::{
    FontLibrary, Frame, Rendered, RenderedStatus, Renderer, Shadow, StatusCell, Theme, UiFont,
    VoiceFrame, system_fonts,
};

/// UI 线程上共享的渲染器。
pub(super) type SharedPainter = Rc<RefCell<Painter>>;

pub(super) struct Painter {
    /// 渲染器（字体库随它）。
    renderer: Renderer,

    /// 建它时用的字族名（空为系统字体），设置没变就不重建。
    font: String,
}

impl Painter {
    /// `font` 是用户选的字族名，空为系统字体；没装就回到系统字体。
    pub(super) fn new(font: &str) -> Option<Self> {
        let started = std::time::Instant::now();
        let library = if font.is_empty() {
            FontLibrary::system("zh-CN")
        } else {
            let ui_font = UiFont {
                family: font.to_owned(),
                files: system_fonts::family_files(font),
            };
            FontLibrary::with_ui_font("zh-CN", &ui_font)
        };
        let library = match library {
            Ok(library) => library,
            Err(error) => {
                tracing::error!(%error, "渲染器字体库加载失败");
                return None;
            }
        };
        tracing::info!(
            elapsed = ?started.elapsed(),
            font = library.ui_family(),
            "候选窗口与状态条使用青简渲染器"
        );
        Some(Self {
            renderer: Renderer::new(library),
            font: font.to_owned(),
        })
    }

    /// 字体变了就重建渲染器；失败时保留上一套字体。
    pub(super) fn set_font(shared: &SharedPainter, font: &str) {
        let mut painter = shared.borrow_mut();
        if painter.font == font {
            return;
        }
        if let Some(replacement) = Self::new(font) {
            *painter = replacement;
        } else {
            tracing::warn!(font, "候选窗口字体加载失败，保留原字体");
        }
    }

    /// 画一帧候选窗口；`dpi` 96 为 100%。
    pub(super) fn render_frame(
        &mut self,
        frame: &Frame,
        color_scheme: ColorScheme,
        dark: bool,
        dpi: u32,
    ) -> Option<Rendered> {
        let started = std::time::Instant::now();
        let rendered = self
            .renderer
            .render(frame, &theme(color_scheme, dark), scale(dpi), Some(&SHADOW))
            .inspect_err(|error| tracing::warn!(%error, "候选窗渲染失败"))
            .ok()?;
        tracing::debug!(
            elapsed = ?started.elapsed(),
            width = rendered.content_width,
            height = rendered.content_height,
            "候选窗位图已画"
        );
        Some(rendered)
    }

    /// 画在同一个候选 HWND 里的语音态。
    pub(super) fn render_voice(
        &mut self,
        frame: &VoiceFrame,
        color_scheme: ColorScheme,
        dark: bool,
        dpi: u32,
    ) -> Option<Rendered> {
        self.renderer
            .render_voice(frame, &theme(color_scheme, dark), scale(dpi), Some(&SHADOW))
            .inspect_err(|error| tracing::warn!(%error, "语音面板渲染失败"))
            .ok()
    }

    /// 画状态条。
    pub(super) fn render_status(
        &mut self,
        cells: &[StatusCell],
        color_scheme: ColorScheme,
        dark: bool,
        dpi: u32,
    ) -> Option<RenderedStatus> {
        self.renderer
            .render_status(cells, &theme(color_scheme, dark), scale(dpi), Some(&SHADOW))
            .inspect_err(|error| tracing::warn!(%error, "状态条渲染失败"))
            .ok()
    }
}

/// 两个窗口都用渲染器画阴影（分层窗口没有系统阴影），参数与 macOS 面板一致。
const SHADOW: Shadow = Shadow::mac_panel();

fn theme(color_scheme: ColorScheme, dark: bool) -> Theme {
    match color_scheme {
        ColorScheme::Cream => Theme::cream(dark),
        ColorScheme::Zizai => Theme::zizai(dark),
        ColorScheme::Latte => Theme::latte(dark),
        ColorScheme::Forest => Theme::forest(dark),
    }
}

/// 点 → 像素的倍数。
fn scale(dpi: u32) -> f32 {
    dpi.max(96) as f32 / 96.0
}
